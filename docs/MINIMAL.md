...
For the first task, inspect the target repository, read only the relevant Skill on demand, make the smallest safe change, and run a focused check. Record uncertainty instead of inventing requirements. A `debug` -> `verify` sequence is enough for an unknown-root-cause bug; use a third judgment only when it controls a distinct risk.
